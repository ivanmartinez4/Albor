...
static void Cmd_pickup(void)
{
    ...
    else if (InBattlePyramid())
    {
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            else if (species == SPECIES_TEDDIURSA
                && species != SPECIES_EGG
                && heldItem == ITEM_NONE)
            {
            ...
            #endif
            else if (species == SPECIES_MUNCHLAX
                && species != SPECIES_EGG
                && heldItem == ITEM_NONE)
            {
                if ((lvlDivBy10 + 1 ) * 5 > Random() % 100)
