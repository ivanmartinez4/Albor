...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES2), MOVE_FLY) == TRUE) // If Mon can learn HM02 and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES2), MOVE_FLASH) == TRUE) // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
