...
static void DrawOptionMenuChoice(const u8 *text, u8 x, u8 y, u8 style)
{
    if ((y >> 4) - gScroll >= 0 && (y >> 4) - gScroll <= 6)
    {
        ...
        dst[i] = EOS;
        AddTextPrinterParameterized(WIN_OPTIONS, FONT_NORMAL, dst, x, (y + 1) - (gScroll << 4), TEXT_SKIP_DRAW, NULL);
    }
}
...
static void DrawOptionMenuTexts(void)
{
    ...
    for (i = 0; i < MENUITEM_COUNT; i++)
    {
        if (i - gScroll >= 0 && i - gScroll <= 6)
            AddTextPrinterParameterized(WIN_OPTIONS, FONT_NORMAL, sOptionMenuItemsNames[i], 8, ((i - gScroll) * 16) + 1, TEXT_SKIP_DRAW, NULL);
    }
}
