...
static const struct SpriteTemplate gSpritePalette_Arrow =
{
...
static void LoadDisplayMonGfx(u16 species, u32 pid)
{
    ...
    {
        pal1 = &gEgg1PaletteTable[sStorage->paletteEgg[0]];
        pal2 = &gEgg2PaletteTable[sStorage->paletteEgg[1]];
    ...
    else if (species != SPECIES_NONE)
    {
        ...
        UniquePalette(sStorage->displayMonPalOffset, species, pid, (bool8)sStorage->palettePokemon[2]);
...
static u16 TryLoadMonIconTiles(u16 species, u32 personality)
{
    ...
    if ((gBaseStats[species].flags & SPECIES_FLAG_GENDER_DIFFERENCE) && GetGenderFromSpeciesAndPersonality(species, personality) == MON_FEMALE)
        species |= 0x8000; // 1 << 15
...
static void CreateBoxScrollArrows(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        u8 spriteId = CreateSprite(&gSpritePalette_Arrow, 92 + i * 136, 28, 22);
        if (spriteId != MAX_SPRITES)
        {
...
{
    u8 spriteId = CreateSprite(&gSpritePalette_Arrow, x, y, subpriority);
    if (spriteId == MAX_SPRITES)
        return NULL;
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            ...
            sStorage->paletteEgg[0] = gBaseStats[GetMonData(mon, MON_DATA_SPECIES)].type1;
            sStorage->paletteEgg[1] = gBaseStats[GetMonData(mon, MON_DATA_SPECIES)].type2;
        }
    }
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            ...
            sStorage->paletteEgg[0] = gBaseStats[GetBoxMonData(boxMon, MON_DATA_SPECIES)].type1;
            sStorage->paletteEgg[1] = gBaseStats[GetBoxMonData(boxMon, MON_DATA_SPECIES)].type2;
        }
    }
    else
    {
...
static bool8 MultiMove_Cancel(void)
{
    ...
    case 2:
        if (!IsDma3ManagerBusyWithBgCopy())
        {
            ...
            LoadPalette(GetTextWindowPalette(), 0xD0, 0x20);
            ShowBg(0);
            return FALSE;
        }
        break;
