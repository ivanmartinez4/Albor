...
static void ShowResults(void)
{
    ...
    case 2:
        ...
        for (i = 0; i < numPlayers; i++)
        {
            ...
            for (j = 0; j < 4; j++)
            {
                ...
                width = GetStringWidth(FONT_NORMAL, gStringVar7, -1);

                // If player got the most of a berry type, highlight their number in red
                if (maxBerriesPicked == berriesPicked && maxBerriesPicked != 0)
                    AddTextPrinterParameterized3(sGfx->windowIds[1], FONT_NORMAL, sResultsXCoords[j] - width, sResultsYCoords[i], sTextColorTable[COLORID_RED], TEXT_SKIP_DRAW, gStringVar7);
                else
                    AddTextPrinterParameterized(sGfx->windowIds[1], FONT_NORMAL, gStringVar7, sResultsXCoords[j] - width, sResultsYCoords[i], TEXT_SKIP_DRAW, NULL);
            }
        }
    ...
    case 9:
        ...
        AddTextPrinterParameterized(sGfx->windowIds[1], FONT_NORMAL, gStringVar7, 0, 1, TEXT_SKIP_DRAW, NULL);
        ...
        if (prizeState != PRIZE_RECEIVED && prizeState != NO_PRIZE)
        {
            ...
            AddTextPrinterParameterized(sGfx->windowIds[1], FONT_NORMAL, gStringVar7, 0, 41, TEXT_SKIP_DRAW, NULL);
        }
        CopyWindowToVram(sGfx->windowIds[0], COPYWIN_GFX);
        CopyWindowToVram(sGfx->windowIds[1], COPYWIN_GFX);
        sGfx->state++;
